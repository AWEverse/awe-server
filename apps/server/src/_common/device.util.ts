...
export default class DeviceUtil {
  ...
  static generateFingerprint(req: Request): string {
    ...
    const allUnknown = Object.values(data).every(
      (val) => val === DeviceUtil.UNKNOWN || val === '0',
    );
    const entropySource = allUnknown
      ? `${DeviceUtil.DELIMITER}uuid=${uuidv4()}`
      : '';
  ...
  static generateEnhancedFingerprint(
    ...
    return createHash(DeviceUtil.HASH_ALGORITHM).update(raw).digest('hex');
  }
